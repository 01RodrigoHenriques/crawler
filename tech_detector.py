...
class TechnologyDetector:
    ...
    def _detect_from_headers(self, headers: Dict) -> List[str]:
        ...
        important_headers = [
            ...
            "x-wp-super-cache", "cf-ray", "x-aspnet-version", "x-iis-version",
            "x-ua-compatible", "strict-transport-security"
        ]
    ...
    def _check_specific_endpoints(self) -> List[str]:
    ...
    def detect(self) -> List[Dict]:
        ...
        try:
            ...
            for tech, category, value in self._detect_from_headers(response.headers):
                techs_detected.add((tech, category))
                logger.debug(f"    OK {tech} ({category})")
            
            # 2. Detectar via HTML
            logger.debug("  Analisando HTML...")
            for tech, category, source in self._detect_from_html(response.text):
                techs_detected.add((tech, category))
                logger.debug(f"    OK {tech} ({category})")
            
            # 3. Verificar endpoints específicos
            logger.debug("  Verificando endpoints específicos...")
            for tech, category, endpoint in self._check_specific_endpoints():
                techs_detected.add((tech, category))
